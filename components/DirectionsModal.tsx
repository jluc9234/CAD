...
const DirectionsModal: React.FC<DirectionsModalProps> = ({ isOpen, onClose, location }) => {
      ...
      const fetchCoordinatesAndInitMap = async () => {
        try {
          if (!mapboxAccessToken) {
            // Throw an error if the token is not configured
            throw new Error("Mapbox token is not configured. Map will not load.");
          }

          const geocodingUrl = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(location)}.json?access_token=${mapboxAccessToken}`;
  ...
  }, [isOpen, location, mapboxAccessToken]);
